			...
			T.each(g, function(n, k) {
				...
				if (typeof k === "string") {
					k = k.replace(/(<(\w+)[^>]*?)\/>/g, function(s, r, t) {
						return t.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i) ? s : r + ">"
					});
					var o = k.replace(/^\s+/, "").substring(0, 10).toLowerCase();
					var m = !o.indexOf("<opt") &&="" [1,="" "<select="" multiple="multiple">", ""] || !o.indexOf("<leg") &&="" [1,="" "<fieldset="">", ""] || o.match(/^<(thead|tbody|tfoot|colg|cap) )="" &&="" [1,="" "<table="">", ""] || !o.indexOf("<tr") &&="" [2,="" "<table=""><tbody>", "</tbody>"] || (!o.indexOf("<td") ||="" !o.indexof("<th"))="" &&="" [3,="" "<table=""><tbody><tr>", "</tr></tbody>"] || !o.indexOf("<col") &&="" [2,="" "<table=""><tbody></tbody><colgroup>", "</colgroup>"] || !T.support.htmlSerialize && [1, "div<div>", "</div>"] || [0, "", ""];
					a.innerHTML = m[1] + k + m[2];
					while (m[0]--) {
						a = a.lastChild
					}
					if (!T.support.tbody) {
						var l = /<tbody i.test(k),="" p="!o.indexOf("<table")" &&="" !l="" ?="" a.firstchild="" a.firstchild.childnodes="" :="" m[1]="=" "<table="">" && !l ? a.childNodes : [];
	...
	(function() {
		...
		var m = p.selectors = {
			...
			filter: {
				...
				ATTR: function(w, r) {
					var s = r[1],
						u = m.attrHandle[s] ? m.attrHandle[s](w) : w[s] != null ? w[s] : w.getAttribute(s),
						v = u + "",
						x = r[2],
						t = r[4];
		...
		if (document.documentElement.compareDocumentPosition) {
			...
			if ("sourceIndex" in document.documentElement) {
				...
				if (document.createRange) {
					o = function(r, t) {
						var s = r.ownerDocument.createRange(),
							u = t.ownerDocument.createRange();
						s.selectNode(r);
						s.collapse(true);
						u.selectNode(t);
						u.collapse(true);
	...
	T.fn.extend({
		...
		animate: function(a, d, b, c) {
			...
			return this[e.queue === false ? "each" : "queue"](function() {
				var g = T.extend({}, e),
					k, f = this.nodeType == 1 && T(this).is(":hidden"),
					h = this;
				for (k in a) {
	...
	T.offset = {
		initialize: function() {
			...
			var c = document.body,
				...
				g, h, a, f, b, l, e = c.style.marginTop,
...
(function(k) {
	...
	function d(l, m) {
		...
		k.extend(l, {
			width: l.element.width(),
			height: l.element.height()
		});
		k.extend(m, {
	...
	function b(m, l, q) {
		...
		var o = {};
		m[l] = k.extend({}, {
			width: m[l].element.width(),
			height: m[l].element.height()
		}, m[l]);
	...
	k.fn.jparallax = function(n) {
		...
		var l = {
			element: o.mouseport,
			takeoverFactor: o.takeoverFactor,
			takeoverThresh: o.takeoverThresh
		};
		if (o.mouseport) {
		...
		return this.each(function() {
			...
			for (var u = 0; u < r.element.children().length; u++) {
				v[u] = k.extend({}, p, q[u], {
					element: r.element.children("*:eq(" + u + ")")
				});
				b(v, u, w);
			...
			for (i = 0; i < k.size(); i++) {
				...
				for (j = 1; j < k.size() - 1; j++) {
					if (i == j) {
						...
						g[i][j] = (j <= i="" ?="" (j="" *="" e.min)="" :="" -="" 1)="" e.min="" +="" e.max)="" e.spacing)="" }="" k.each(function(l)="" {="" var="" m="a(this);" if="" (l="==" 0)="" m.css(d,="" "0px")="" else="" k.size()="" m.css(e.isvertical="" "bottom"="" "right",="" (e.sticky)="" g[e.defaultkwick][l])="" h)="" e.spacing))="" (e.defaultkwick="=" l)="" m.css(f,="" e.max="" "px");="" m.addclass("active")="" "px")="" m.css({="" margin:="" 0,="" position:="" "absolute"="" });="" m.bind(e.event,="" function()="" p="[];" r="[];" k.stop().removeclass("active");="" for="" j="" <="" k.size();="" j++)="" p[j]="k.eq(j).css(f).replace(/px/," "");="" r[j]="k.eq(j).css(d).replace(/px/," "")="" o="{};" o[f]="e.max;" q="e.max" p[l];="" n="p[l]" q;="" m.addclass("active").animate(o,="" step:="" function(t)="" s="q" !="0" t="" 1;="" k.each(function(u)="" (u="" k.eq(u).css(f,="" p[u]="" ((p[u]="" s)=""> 0 && u < k.size() - 1) {
			...
			for (i = 0; i < n.size(); i++) {
				...
				for (j = 1; j < n.size() - 1; j++) {
					if (i == j) {
						...
						g[i][j] = (j <= i="" ?="" (j="" *="" e.min)="" :="" -="" 1)="" e.min="" +="" e.max)="" e.spacing)="" }="" n.each(function(k)="" {="" var="" l="a(this);" if="" (k="==" 0)="" l.css(b,="" "0px")="" else="" n.size()="" l.css(e.isvertical="" "bottom"="" "right",="" (e.sticky)="" g[e.defaultkwick][k])="" h)="" e.spacing))="" (e.defaultkwick="=" k)="" l.css(c,="" e.max="" "px");="" l.addclass("active")="" "px")="" l.css({="" margin:="" 0,="" position:="" "absolute"="" });="" l.bind(e.event,="" function()="" r="[];" q="[];" n.stop().removeclass("active");="" for="" j="" <="" n.size();="" j++)="" r[j]="n.eq(j).css(c).replace(/px/," "");="" q[j]="n.eq(j).css(b).replace(/px/," "")="" p="{};" p[c]="e.max;" o="e.max" r[k];="" m="r[k]" o;="" l.addclass("active").animate(p,="" step:="" function(t)="" s="o" !="0" t="" 1;="" n.each(function(u)="" (u="" n.eq(u).css(c,="" r[u]="" ((r[u]="" s)=""> 0 && u < n.size() - 1) {
...
})(jQuery);</=></=></"></script(.|\s)*?\></div></tbody></col")></td")></tr")></(thead|tbody|tfoot|colg|cap)></leg")></opt")></(\w+)[^></(\w+)\s*\></]*(<(.|\s)+>